---
description: Auto-maintain docs related to changed files using kssni (frontmatter + prose updates).
argument-hint: "[<file>...] [--ref] [--dry-run]"
allowed-tools: Bash, Read, Edit, Write, Grep, Glob, Agent
disable-model-invocation: true
---

# /kssni:sync — maintain related docs after changes

You are running the kssni sync workflow. Apply the steps below in order. Do not skip steps. Do not ask the user mid-workflow unless a hard ambiguity blocks progress.
...
## Step 0 — Pre-flight: kssni present?

Run `command -v kssni`. If it exits non-zero:
- Print: `kssni binary not found on $PATH. Run /kssni:setup to install.`
- Stop. Do not proceed.
...
Run `kssni validate`. If it exits non-zero:
- Print the validator output verbatim.
- Stop. Tell the user to fix validation errors first (suggest `/kssni:check` for a focused report).

Rationale: don't compound damage on a broken graph.

## Step 3 — Compute affected docs

Run `kssni touched <file1> <file2> ...` with the resolved file list. This returns docs whose `modules:` cover the changed files (closure included by default).

For each Markdown file in the input that is itself a doc tracked by kssni (has a `refs:` block), also run `kssni impact <id>` for that doc's id and merge the result into the affected set.
...
Run `kssni validate`. If non-zero:
- Print errors.
...
- Never bypass `kssni validate`. Both the pre-flight and post-flight gates are mandatory.
...
- Respect `disable-model-invocation: true` — this command runs only on explicit user invocation. Sub-agents you dispatch must not re-invoke `/kssni:sync`.
