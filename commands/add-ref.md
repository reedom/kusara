---
description: Author or update the kssni `refs:` frontmatter block on a Markdown file.
argument-hint: "<file> [--kind <kind>] [--id <id>]"
allowed-tools: Bash, Read, Edit, Write, Grep, Glob
disable-model-invocation: true
---

# /kssni:add-ref — guided frontmatter authoring
...
4. Determine the target kind:
   ...
   - Else, infer from the file's path against the `path_globs` in `${KSSNI_DOC_ROOT}/kinds.md` (default `docs/kinds.md`). If no match, ask the user once for the kind.
5. Determine the `id`:
   - If `--id` provided, use it.
   - Else derive from the kind's `id_pattern` and the file's slug.
   - Verify uniqueness: run `kssni list` and confirm the id is not present.
...
8. Run `kssni validate`. If it fails on the edited file, revert via `git checkout -- <file>` and report the error.
9. Print the resulting `refs:` block.

Constraints:
- Never set `generated:` or `indexes_kind:` — those are written by `kssni index`.
- Do not edit any file other than `<file>`.
- Do not run `kssni index` or `kssni index map`.
